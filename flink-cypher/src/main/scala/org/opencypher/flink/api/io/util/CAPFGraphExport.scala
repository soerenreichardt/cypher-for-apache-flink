...
import org.opencypher.flink.api.io.{GraphElement, Relationship}
...
object CAPFGraphExport {
  ...
  implicit class CanonicalTableFlinkSchema(val schema: PropertyGraphSchema) extends AnyVal {

    def canonicalNodeFieldReference(labels: Set[String]): Seq[ResolvedFieldReference] = {
      val id = ResolvedFieldReference(GraphElement.sourceIdKey, Types.LONG)
  ...
  implicit class CanonicalTableExport(graph: RelationalCypherGraph[FlinkTable]) {
    ...
    def canonicalNodeTable(labels: Set[String]): Table = {
      ...
      val idRenaming = header.column(v) -> GraphElement.sourceIdKey
