...
case class CAPFRecordsFactory()(implicit capf: CAPFSession) extends RelationalCypherRecordsFactory[FlinkTable] {
  ...
  override def fromElementTable(elementTable: ElementTable[FlinkTable]): CAPFRecords = {
    val withCypherCompatibleTypes = elementTable.table.table.withCypherCompatibleTypes
    CAPFRecords(elementTable.header.withCharacterEscaping(escaping), withCypherCompatibleTypes)
  }
  ...
  override def from(
    ...
    CAPFRecords(header.withCharacterEscaping(escaping), table, displayNames)
  }
