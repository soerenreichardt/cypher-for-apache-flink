...
import org.opencypher.flink.api.value.CAPFRelationship
import org.opencypher.flink.test.CAPFTestSuite
import org.opencypher.okapi.api.value.CypherValue._
import org.opencypher.okapi.testing.Bag

class BoundedVarExpandTests extends CAPFTestSuite with ScanGraphInit {
  ...
  ignore("var expand return var length rel as list of relationships") {
    // Given
    val given = initGraph("CREATE (a:Node {v: 'a'})-[:REL]->(:Node {v: 'b'})-[:REL]->(:Node {v: 'c'})-[:REL]->(a)")
    ...
    val rel1 = CAPFRelationship(2, 0, 1, "REL")
    val rel2 = CAPFRelationship(4, 1, 3, "REL")
    val rel3 = CAPFRelationship(5, 3, 0, "REL")
